...
class TrainingConfig:
    excel_filename: str = '01. 시설물 50개 샘플 데이터.xlsx'
    ...
    raw_sheet: str = '수집원본데이터'
    info_sheet: str = '설치 및 분석정보'
    feature_name_map: Dict[str, str] = field(default_factory=lambda: {
        '방식전위(mV)'  : '방식전위',
        'AC유입(mV)'    : 'AC유입',
        '희생전류(mA)'  : '희생전류',
        '온도(℃)'      : '온도',
...
RAW_SHEET  = DEFAULT_CONFIG.raw_sheet
INFO_SHEET = DEFAULT_CONFIG.info_sheet
FEATURE_NAME_MAP = DEFAULT_CONFIG.feature_name_map
...
def load_install_info(excel_path: str, config: TrainingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    raw           = pd.read_excel(excel_path, sheet_name=config.info_sheet, header=None)
    ...
    rename_map = {}
    for c in df.columns:
        if '장비번호' in c:  rename_map[c] = '장비번호'
        elif '시설번호' in c: rename_map[c] = '시설번호'
        elif 'CTN' in c:      rename_map[c] = 'CTN'
        elif '형식' in c:     rename_map[c] = '형식'
        elif '주소' in c:     rename_map[c] = '주소'
        elif '위도' in c:     rename_map[c] = '위도'
        elif '경도' in c:     rename_map[c] = '경도'

    df = df.rename(columns=rename_map)
    if '장비번호' not in df.columns:
        raise ValueError("'설치 및 분석정보' 시트에서 장비번호 컬럼을 찾지 못했습니다.")

    keep = [c for c in ['장비번호', '시설번호', 'CTN', '형식', '주소', '위도', '경도']
            if c in df.columns]
    df = df[keep].copy()
...
def load_raw_collection_data(excel_path: str, config: TrainingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    raw        = pd.read_excel(excel_path, sheet_name=config.raw_sheet, header=None)
    device_row = raw.iloc[1].tolist()
    metric_row = raw.iloc[2].tolist()
    data       = raw.iloc[3:].copy().reset_index(drop=True)

    time_col = normalize_text(device_row[0]) or '일자'
    data     = data.rename(columns={0: time_col})
    data[time_col] = pd.to_datetime(data[time_col], errors='coerce')
    data     = data.dropna(subset=[time_col]).copy()

    long_frames, col_idx = [], 1
    n_cols = raw.shape[1]
    ...
    while col_idx < n_cols:
        ...
        device_id, fac_no = parse_device_header(header_text)
        block_cols = list(range(col_idx, min(col_idx + len(config.all_raw_features), n_cols)))

        feature_map = {}
        for c in block_cols:
            metric  = normalize_text(metric_row[c])
            canon   = config.feature_name_map.get(metric)
            if canon:
                feature_map[c] = canon

        if feature_map:
            subset = data[[time_col] + list(feature_map.keys())].copy()
            subset = subset.rename(columns={time_col: '측정시각', **feature_map})
            subset['장비번호']    = device_id
            subset['시설번호_raw'] = fac_no
            for f in config.all_raw_features:
                if f not in subset.columns:
                    subset[f] = np.nan
                subset[f] = pd.to_numeric(subset[f], errors='coerce')
            subset = subset[['측정시각', '장비번호', '시설번호_raw'] + config.all_raw_features]
            long_frames.append(subset)

        col_idx += len(config.all_raw_features)

    if not long_frames:
        raise ValueError("'수집원본데이터' 시트에서 장비 블록을 찾지 못했습니다.")

    merged = pd.concat(long_frames, ignore_index=True)
    return merged.sort_values(['장비번호', '측정시각']).reset_index(drop=True)


def build_master_dataset(file_path: str, config: TrainingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    path_lower = file_path.lower()
    
    # Parquet 파일 직접 로드 (가장 권장)
    if path_lower.endswith('.parquet'):
        print(f"  -> Parquet 형식 직접 로드 중: {file_path}")
    ...
    # CSV 파일 직접 로드
        ...
        print(f"  -> CSV 형식 직접 로드 중: {file_path}")
        df = pd.read_csv(file_path)
        if '측정시각' in df.columns:
            df['측정시각'] = pd.to_datetime(df['측정시각'], errors='coerce')
        df = apply_comm_quality_filter(df, config)
        return df
        
    # 기존 Excel 로직
    raw_df  = load_raw_collection_data(file_path, config)
    info_df = load_install_info(file_path, config)
...
def save_common_artifacts(
    ...
    artifact_config = {
        ...
        'raw_sheet'             : config.raw_sheet,
        'info_sheet'            : config.info_sheet,
        'sacrificial_devices'   : list(config.sacrificial_devices),
        'sacrificial_features'  : list(config.sacrificial_features),
...
def main(config: TrainingConfig = DEFAULT_CONFIG):
    ...
    # ══════════════════════════════════════════════════════════
    # STEP 1 ─ 데이터 로드 (공통 모델 + 희생양극 모델이 공유)
    # ══════════════════════════════════════════════════════════
    print('=' * 55)
    print(' STEP 1 : 데이터 로드')
    print('=' * 55)
    master_df = build_master_dataset(str(excel_path), config)
